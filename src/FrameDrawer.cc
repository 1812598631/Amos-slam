...
cv::Mat FrameDrawer::DrawFrame()
{
    ...
    else if(state==Tracking::OK) //TRACKING
    {
        ...
        //绘制动态特征点
        /*
        for(int i=0;i<mvDynaCurrentKeys.size();i++)
        {
            //在特征点附近正方形选择四个点
            cv::Point2f pt1,pt2;
            pt1.x=mvDynaCurrentKeys[i].pt.x-r;
            pt1.y=mvDynaCurrentKeys[i].pt.y-r;
            pt2.x=mvDynaCurrentKeys[i].pt.x+r;
            pt2.y=mvDynaCurrentKeys[i].pt.y+r;

            cv::rectangle(im,pt1,pt2,cv::Scalar(0,0,255));
            cv::circle(im,mvDynaCurrentKeys[i].pt,2,cv::Scalar(0,0,255),-1);
        }
         */
    }

    cv::imwrite("output/" + to_string(mnid) + "_keys.png", im);
    //然后写入状态栏的信息
    cv::Mat imWithInfo;
...
cv::Mat FrameDrawer::DivisionDrawer(){
    cv::Mat im;
//不同Mask超像素点的绘制
    color.copyTo(im);
    //cvtColor(im,im,CV_GRAY2BGR);
    ...
    for(it=mvFlow_3d.begin();it!=mvFlow_3d.end();it++)
    {
        if (!it->empty())
         points.push_back(Point2i(it->at<float>(0), it->at<float>(1)));
        }
        Scalar color=Scalar(rand()%255, rand()%255, rand()%255);
        for(int j = 0; j < points.size(); j++)
            circle(im, points[j], 3, color, -1);//画点，其实就是实心圆
    points.clear();
...
void FrameDrawer::Update(Tracking *pTracker)
{
    ...
    mvFlow_3d=pTracker->mCurrentFrame.vFlow_3d;
    mvT_M=pTracker->mCurrentFrame.T_M;
    mvDynaCurrentKeys=pTracker->mCurrentFrame.mvDynaKeys;
