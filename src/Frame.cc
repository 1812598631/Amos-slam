					   ...
					   const cv::Mat &im)	//等待提取特征点的图像
{
    ...
    void Frame::CalNorm(const cv::Mat &imRGB, const cv::Mat &imGray,const cv::Mat &imDepth,const cv::Mat &imS, Frame &lastframe)
    {
        cv::Mat imD=imDepth;
        ...
        if(mvKeys.empty())
            return ;
        UndistortKeyPoints();
        ComputeStereoFromRGBD(imD);
        mvpMapPoints = vector<MapPoint*>(N,static_cast<MapPoint*>(NULL));
        mvbOutlier = vector<bool>(N,false);

        AssignFeaturesToGrid();
    }
    cv::Mat Frame::CalDyna(const cv::Mat &imRGB, const cv::Mat &imGray,const cv::Mat &imDepth,const cv::Mat &imS, Frame &lastframe)
    {
        ///去除动态特征
        //几何分割
        vector<center> centers;
        cv::Mat DynRGB=imRGB.clone(),imD=imDepth;
        cv::Mat SegMask=cv::Mat::zeros(640,480,CV_8UC1);
        bool SegisMove = 0;
        int k=15;
        cluster *a=new cluster(DynRGB,imDepth,centers,k);

        imD.convertTo(  //将图像转换成为另外一种数据类型,具有可选的数据大小缩放系数
                imD,            //输出图像
                CV_32F,             //输出图像的数据类型
                0.0002);   //缩放系数
        cv::Mat labelMask=a->labelMask;//获取每个像素的Label
        vector<int> epNum(k,0);
        vector<int> rmVector(k,0);
        // *根据场景流判断动态性
        /*

        for (int i=0;i<vFlow_3d.size();i++)
        {
            float u,v;
            double label;
            u=vFlow_3d[i].at<float>(0);
            v=vFlow_3d[i].at<float>(1);
            u=(int)u;
            v=(int)v;
            label=labelMask.at<double>(v, u);
            labelset.insert(label);
        }

        */

        vector<vector<float>> clusterRpe;
        clusterRpe.resize(k);
        int pixelId;
        for (int i = 0; i < mvMatch.size(); ++i) {
            pixelId=labelMask.at<double>(mvMatch[i].y,mvMatch[i].x);
            clusterRpe[centers[pixelId-1].id].push_back(mvRpe[i]);
        }
        vector<float> AveClusterRpe;
        float sum=0,rmsize=0;
        //cout<<"ave Rpe="<<endl;
        for (int i = 0; i < clusterRpe.size(); ++i) {
            for (int j = 0; j < clusterRpe[i].size(); ++j) {
                sum+=clusterRpe[i][j];
            }
            AveClusterRpe.push_back(sum/clusterRpe[i].size());
            //cout<<sum/clusterRpe[i].size()<<endl;
            sum=0;
        }
///根据对极集合判断动态性
    set<int> labelset;
    set<int> rmCluster_set;
        ...
        {
            rmCluster_set.insert(centers[*it-1].id);
            epNum[centers[*it-1].id]++;
        }
        ///根据对极集合判断动态性
        /*
        for (int i=0;i<T_M.size();i++) {
            ...
            double label;
            set<int> clusterSet;
            label = labelMask.at<double>((int)T_M[i].y, (int)T_M[i].x);
            int epi=centers[label - 1].id;

            for (int m = -15; m < 15; m++) {
                for (int n = -15; n < 15; n++) {
                    int my = ((int) T_M[i].y + n);
                    int mx = ((int) T_M[i].x + m);
                    if (((int) T_M[i].y + n) > (DynRGB.rows - 1)) my = (DynRGB.rows - 1);
                    if (((int) T_M[i].y + n) < 1) my = 0;
                    if (((int) T_M[i].x + m) > (DynRGB.cols - 1)) mx = (DynRGB.cols - 1);
                    if (((int) T_M[i].x + m) < 1) mx = 0;
                    label = labelMask.at<double>(my, mx);
                    if((int)imS.ptr<uchar>((int)my)[(int)mx]==1)
                        SegisMove=1;
                    clusterSet.insert(centers[label - 1].id);
                }
            }
            float max = 0;
            for (auto it = clusterSet.begin(); it != clusterSet.end(); it++) {
                int ci = *it;
                if (AveClusterRpe[ci] > max) {
                    max = AveClusterRpe[ci];
                    epi=ci;
                }
            }
            epNum[epi]++;
        ...
        for(int i=0;i<k;i++)
        {
            //if(epNum[i]>8||epNum[i]>0&&AveClusterRpe[i]>=5)
            if(epNum[i]>0&&AveClusterRpe[i]>=3)
                    rmVector[i]=1;
        }
        //cout<<rmsize<<endl;
        //根据聚类标签汇总重投影误差
        SegisMove=1;
        if(SegisMove==1)//先验动态目标移动
            mvDynaKeys=mpORBextractorLeft->MovingKeyPoints(imGray, imS,labelMask,centers,rmVector, mvbDynaFlag, mvKeysTemp);
        else
            mvDynaKeys=mpORBextractorLeft->MovingKeyPoints(imGray, SegMask,labelMask,centers,rmVector, mvbDynaFlag, mvKeysTemp);
...
//            if(rmVector[i]==1)
//            {
//                for(int j=0;j<a->points[i].size();j++)
//                    circle(DynRGB, a->points[i][j], 3, color, -1);//画点，其实就是实心圆
//            }
//        }
//        if (!T_M.empty()) {
//            Scalar color = cv::Scalar(255, 0, 0);
//            for (int j = 0; j < T_M.size(); j++)
//            {
//                circle(DynRGB, T_M[j], 8, color, 1);//画点，其实就是实心圆
//                circle(DynRGB, T_M[j], 3, color, -1);//画点，其实就是实心圆
//            }
//        }
