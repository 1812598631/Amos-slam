...
def parse_args(argv=None):
    ...
    parser.add_argument('--images', default='input:output', type=str,
...
def save_mask(masks,mask_path):
    ...
    cv2.imwrite(name, masks)

def prep_display(dets_out, img, h, w,mask_path, undo_transform=True, class_color=False, mask_alpha=0.45, fps_str=''):
    """
...
def yolact__init():
    ...
    parse_args()
    print("[python] over")
    str_temp=SavePath.from_str('weights/yolact_resnet50_54_800000.pth')
    str_temp = str_temp.model_name + '_config'
    set_cfg(str_temp)
    with torch.no_grad():
            ...
            net = Yolact()
            state_dict = torch.load('weights/test_yolact_resnet50_54_800000.pth')
            """
            ...
            for key in list(state_dict.keys()):
                ...
                if key.startswith('fpn.downsample_layers.'):
                    if cfg.fpn is not None and int(key.split('.')[2]) >= cfg.fpn.num_downsample:
                        del state_dict[key]
            torch.save(state_dict,'weights/test_yolact_resnet50_54_800000.pth')
            print('save over')
            """
