    ...
    const double &timestamp)        //时间戳
{
    ...
    cv::Mat imDepth = imD;
    cv::Mat imDT=imD;
    cv::Mat SegMask=cv::Mat::zeros(640,480,CV_8UC1);
    // step 1：将RGB或RGBA图像转为灰度图像
    ...
    if (imDPre.empty())
        imDPre=imD;
/*
    bool needNewKF = NeedNewKeyFrame();
    if(!needNewKF)
    {
        mCurrentFrame = Frame(
                mImGray,                //灰度图像
                ...
                mbf,                    //相机基线*相机焦距
                mThDepth);
        mCurrentFrame.CalNorm(imRGB,mImGray,imDT,SegMask,mLastFrame);
        Track();
        //返回当前帧的位姿
        //cout<<"final_mTcw"<<endl<<mCurrentFrame.mTcw<<endl;
        imPre = imRGB;
        imDPre = imDepth;
        return mCurrentFrame.mTcw.clone();
...
//    if(needNewKF) {
        ...
        if (!mLastFrame.mTcw.empty()) {
            ...
            /*
            while(!isNewImgArrived())
            {
                usleep(1);
            }
            SegMask=mpSeg->mMask;
             */
            GetSceneFlowObj(SegMask, imPre, imDPre, imRGB, imDepth);
            //GetRpe(mpLastKeyFrame,imRGB);
        }

        mpFrameDrawer->color = mCurrentFrame.CalDyna(imRGB, mImGray, imDT, SegMask, mLastFrame);

        if (!imRGB.empty() && !mpFrameDrawer->color.empty()) {
            cv::imwrite("output/" + to_string(mCurrentFrame.mnId) + "_rgb.png", imRGB);
            cv::imwrite("output/" + to_string(mCurrentFrame.mnId) + "_seg.png", mpFrameDrawer->color);
            cv::imwrite("output/" + to_string(mCurrentFrame.mnId) + "_mask.png", SegMask);
        }
    ...
    else
    {
        ...
        if(!mbOnlyTracking)
        {
            ...
            if(mState==OK)
            {
                ...
                // 运动模型是空的或刚完成重定位
                // mCurrentFrame.mnId < mn
                // LastRelocFrameId + 2 这个判断不应该有
        ...
        if(bOK)
        {
            ...
            if(!mLastFrame.mTcw.empty())
            {
                ...
                mVelocity = mCurrentFrame.mTcw*LastTwc; // 其实就是 c1为上一帧的相机坐标系 c2当前帧坐标系 mVelocity=Tc2w * Twc1 =Tc2c1 也就是 上一帧到当前帧的变换位姿
            }
            else
                //否则生成空矩阵
                mVelocity = cv::Mat();
...
void Tracking::SetSegment(yolact* segment)
{
        mpSeg=segment;
}
void Tracking::GetImg(const cv::Mat& img)
{
        unique_lock<mutex> lock(mpSeg->mMutexGetNewImg);
        mpSeg->mbNewImgFlag=true;
        img.copyTo(mpSeg->mImg);
}
bool Tracking::isNewImgArrived()
{
        unique_lock<mutex> lock(mpSeg->mMutexGetNewImg);
        if(mbNewImgFlag)
        {
...
void Tracking::GetSceneFlowObj(
         ...
         cv::Mat imdepth)
    {
        ...
        {
            if(mbRGB)
                cvtColor(imRGB,gray,CV_RGB2GRAY);
            else
                cvtColor(imRGB,gray,CV_BGR2GRAY);
        }
        else if(imRGB.channels()==4)
        {
            if(mbRGB)
                cvtColor(imRGB,gray,CV_RGBA2GRAY);
            else
                cvtColor(imRGB,gray,CV_BGRA2GRAY);
        }

        if(imlast.channels()==3)
        {
            if(mbRGB)
                cvtColor(imlast,imlast,CV_RGB2GRAY);
            else
                cvtColor(imlast,imlast,CV_BGR2GRAY);
        }
        else if(imlast.channels()==4)
        {
            if(mbRGB)
                cvtColor(imlast,imlast,CV_RGBA2GRAY);
            else
                cvtColor(imlast,imlast,CV_BGRA2GRAY);
        }

        cv::goodFeaturesToTrack(imlast, prepoint, 1000, 0.01, 8, cv::Mat(), 3, true, 0.04);
        cv::cornerSubPix(imlast, prepoint, cv::Size(10, 10), cv::Size(-1, -1), cv::TermCriteria(CV_TERMCRIT_ITER | CV_TERMCRIT_EPS, 20, 0.03));
        cv::calcOpticalFlowPyrLK(imlast, gray, prepoint, nextpoint, state, err, cv::Size(22, 22), 5, cv::TermCriteria(CV_TERMCRIT_ITER | CV_TERMCRIT_EPS, 20, 0.01));

        std::vector<cv::Point2f> match_pre,match_cur;
        ...
        for(int i=0;i<state.size();i++)
        {
            ...
            double sum_check = 0;
            for (int j = 0; j < 9; j++)
                sum_check += abs(imlast.at<uchar>(y1 + dy[j], x1 + dx[j]) - gray.at<uchar>(y2 + dy[j], x2 + dx[j]));
            if (sum_check > 2520) state[i] = 0;
            sta_label=(int)segMask.ptr<uchar>((int)nextpoint[i].y)[(int)nextpoint[i].x];
        ...
        cv::Mat F = cv::findFundamentalMat(match_pre, match_cur, cv::FM_RANSAC, 0.1, 0.99);
        ///第一次计算基础矩阵 根据极限约束 去掉不满足约束点后再次进行基础矩阵的计算和极线约束
        for (int i = 0; i < prepoint.size(); i++)
        {
            if (state[i] != 0)//光流成功追踪，harris角点成功匹配
            {
        ...
        F = cv::findFundamentalMat(F_prepoint, F_nextpoint,  cv::FM_RANSAC, 0.1, 0.99);

/*
        cv::Mat essential_matrix=findEssentialMat (match_pre, match_cur, mK, RANSAC);
        cv::Mat R,t,T=mCurrentFrame.mTcw.clone();
        recoverPose(essential_matrix, match_pre, match_cur, mK, R,t);
...
*/
        ...
        std::vector<cv::Point3f> cur_3d(N);

        for(int i=0;i<N;i++) {
            float z1 = imDlast.at<float>(match_pre[i].y, match_pre[i].x);
            float z2 = imdepth.at<float>(match_cur[i].y, match_cur[i].x);
            if (z1 > 0 && z2 > 0) {
                float x = (match_pre[i].x - mCurrentFrame.cx) * z1 * mCurrentFrame.invfx;
                ...
                x3Dp =Rwl*x3Dp+twl;
                //x3Dp = mLastFrame.mRwc * x3Dp + mLastFrame.mOw;
...
//            cv::solvePnP(pre_3d, cur_2d, camera_mat, distCoeffs, Rvec, Tvec);
            cv::Rodrigues(Rvec, d);

    cv::Mat mTcw;
    if(mpLastKeyFrame)
    computeMtcwUseLK(mpLastKeyFrame, imRGB,  mCurrentFrame.mnId - mpLastKeyFrame->mnFrameId ==1, mK, mDistCoef, mTcw, mnMatchesInliers);
    ;
    cv::Mat Mod = cv::Mat::eye(4,4,CV_32F);//初始估计模型
            ...
            std::vector<int> PNP_inlier,MM_inlier;
            cv::Mat MotionModel;
            if (mVelocity.empty())
            ...
            vector<float> RpePNP;
            vector<float> RpeMotion;
    for (int i = 0; i < pre_3d.size(); ++i) {
                if (pre_3d[i].z > 0&&(cur_2d[i].x!=0&&cur_2d[i].y!=0)) {
                    const cv::Mat x3D = (cv::Mat_<float>(3, 1) << pre_3d[i].x, pre_3d[i].y, pre_3d[i].z);
                    cv::Mat x3D_c,R,t;
                    if(!mTcw.empty())
                    {
                        R=mTcw.rowRange(0, 3).colRange(0, 3);
                        t=mTcw.rowRange(0, 3).col(3);
                    }
                    else
                    {
                        R=Mod.rowRange(0, 3).colRange(0, 3);
                        t=Mod.rowRange(0, 3).col(3);
                    }
                    /*
    ...
    for (int i = 0; i < pre_3d.size(); ++i) {
        if (pre_3d[i].z > 0&&(cur_2d[i].x!=0&&cur_2d[i].y!=0)) {
            ...
            R=MotionModel.rowRange(0, 3).colRange(0, 3);
            t=MotionModel.rowRange(0, 3).col(3);
            ...
            */
            ...
            cv::Mat output;
            //cout<<"PNPinliers:"<<PNP_inlier.size()<<endl<<"MM_inliers:"<<MM_inlier.size()<<endl;
            if (PNP_inlier.size()>=MM_inlier.size())
            {
                // save the inliers IDs
                output = Mod;
                mCurrentFrame.mvRpe=RpePNP;
            }
            else
            {
                output = MotionModel;
                mCurrentFrame.mvRpe=RpeMotion;
            }

        //cout<<"Mod Tcw="<<endl<<Mod<<endl;
        //cout<<"MotionalModel Tcw="<<endl<<output<<endl;
        //cout<<"LK:"<<output<<endl;
        mCurrentFrame.SetPose(output);
        mCurrentFrame.mvepipolar.resize(prepoint.size(),0);
        for (int i = 0; i < prepoint.size(); i++)
        {
            if (state[i] != 0)//光流成功追踪，harris角点成功匹配
            {
                ...
                double C = F.at<double>(2, 0)*prepoint[i].x + F.at<double>(2, 1)*prepoint[i].y + F.at<double>(2, 2);
                double dd = fabs(A*nextpoint[i].x + B*nextpoint[i].y + C) / sqrt(A*A + B*B);
                mCurrentFrame.mvepipolar[i]=dd;
                // Judge outliers
                if (dd <= 1) continue;
                mCurrentFrame.T_M.push_back(nextpoint[i]);
            }
        }

        ///利用ORB特征点匹配计算场景流，对极几何约束计算
        for (int i =0;i<match_cur.size();i++)
        {
            float z1 = imDlast.at<float>(match_pre[i].y, match_pre[i].x);
            float z2 = imdepth.at<float>(match_cur[i].y, match_cur[i].x);
            if(z1>0&&z2>0) {
                float x = (match_cur[i].x - mCurrentFrame.cx) * z1 * mCurrentFrame.invfx;
                float y = (match_cur[i].y - mCurrentFrame.cy) * z1 * mCurrentFrame.invfy;

                cv::Mat x3Dp = (cv::Mat_<float>(3, 1) << x, y, z2);
                x3Dp = mCurrentFrame.mRwc * x3Dp + mCurrentFrame.mOw;

                cv::Point3f tmp_3d;
                cv::Mat x3Dc = (cv::Mat_<float>(3, 1) << x, y, z2);
                x3Dc = mCurrentFrame.mRwc * x3Dc + mCurrentFrame.mOw;;
                tmp_3d.x = x3Dp.at<float>(0);
                tmp_3d.y = x3Dp.at<float>(1);
                tmp_3d.z = x3Dp.at<float>(2);
                cur_3d[i] = tmp_3d;

                //cout<<"x3Dp="<<x3Dp<<endl<<"x3Dc="<<x3Dc<<endl;
                cv::Point3f flow3d;
                flow3d.x = pre_3d[i].x - cur_3d[i].x;
                flow3d.y = pre_3d[i].y - cur_3d[i].y;
                flow3d.z = pre_3d[i].z - cur_3d[i].z;

                float sf_norm = std::sqrt(flow3d.x * flow3d.x + flow3d.z * flow3d.z);
                cv::Mat SF = (cv::Mat_<float>(3, 1) << match_cur[i].x, match_cur[i].y, sf_norm);

                //cout<<"x="<<SF.at<float>(0)<<"y="<<SF.at<float>(1)<<"sf_norm="<<SF.at<float>(2)<<endl;

                if (sf_norm > 3) {
                    mCurrentFrame.vFlow_3d.push_back(SF);
                    //cout<<"sf_norm="<<SF.at<float>(2)<<endl;
                }
            }
        }
        ///queryIdx保存着图像1匹配点的序号，matches的trainIdx，其保存着图像2匹配点的序号
    }
    ...
    cv::Mat last_color;
cv::Mat Tracking::GetRpe(KeyFrame *lastKeyFrame, Mat color)
    {
        int obsPlus = 0;
...
//        cout<<lastKeyFrame->mvKeysUn.size()<<endl;
        ...
        /**需要用到的数据*/
        vector<cv::Point2f> next_keypoints,pre_keypoints;
        prev_keypoints.clear();
        for ( auto kp:keypoints )
            prev_keypoints.push_back(kp);
        pre_keypoints.resize(prev_keypoints.size());
...
//    cout<<"preKeyPointNum"<<prev_keypoints.size()<<endl;
        ...
        i=0;
        for ( auto iter=pre_keypoints.begin(); iter!=pre_keypoints.end(); i++)
        {
            if ( status[i] == 0 )
            {
                iter = pre_keypoints.erase(iter);
                continue;
            }
            *iter = prev_keypoints[i];//edit keypoints' coordinate
            iter++;
        }
//    cout<<"tracked keypoints: "<<keypoints.size()<<endl;
    ...
    cv::Mat F = cv::findFundamentalMat(pre_keypoints, keypoints, cv::FM_RANSAC, 0.1, 0.99);
    mCurrentFrame.mvepipolar.resize(pre_keypoints.size(),0);
    for (int i = 0; i < pre_keypoints.size(); i++)
    {

            double A = F.at<double>(0, 0)*pre_keypoints[i].x + F.at<double>(0, 1)*pre_keypoints[i].y + F.at<double>(0, 2);
            double B = F.at<double>(1, 0)*pre_keypoints[i].x + F.at<double>(1, 1)*pre_keypoints[i].y + F.at<double>(1, 2);
            double C = F.at<double>(2, 0)*pre_keypoints[i].x + F.at<double>(2, 1)*pre_keypoints[i].y + F.at<double>(2, 2);
            double dd = fabs(A*keypoints[i].x + B*keypoints[i].y + C) / sqrt(A*A + B*B);
            mCurrentFrame.mvepipolar[i]=dd;
            // Judge outliers
            if (dd <= 1.5) continue;
            mCurrentFrame.T_M.push_back(keypoints[i]);
    }
        ...
        mCurrentFrame.mvMatch.resize(keypoints.size());
        mCurrentFrame.mvRpe.resize(keypoints.size());
        if(!(mappointInCurrentFrame.empty()||keypoints.empty()||mDistCoef.empty()))
        {
            ...
            cv::solvePnPRansac(mappointInCurrentFrame, keypoints, mK, mDistCoef , R_vector, T, false, 50,3, 0.98, ransacInlier, SOLVEPNP_ITERATIVE);

            cv::Rodrigues(R_vector, R);
            Mat_<double> Rt = (Mat_<double>(4, 4) <<
            ...
            for(auto iter=mappointInCurrentFrame.begin();iter!=mappointInCurrentFrame.end();i++){
                cv::Point3f pt=*iter;
                cv::Mat x3Dp = (cv::Mat_<float>(3, 1) << pt.x, pt.y, pt.z);
                x3Dp=Rt_float.rowRange(0, 3).colRange(0, 3) * x3Dp + Rt_float.rowRange(0, 3).col(3);
                const float xc = x3Dp.at<float>(0);
                const float yc = x3Dp.at<float>(1);
                const float invzc = 1.0 / x3Dp.at<float>(2);

                const float u = mCurrentFrame.fx * xc * invzc + mCurrentFrame.cx;
                const float v = mCurrentFrame.fy * yc * invzc + mCurrentFrame.cy;
                const float u_ = keypoints[i].x - u;
                const float v_ = keypoints[i].y - v;
                const float Rpe = std::sqrt(u_ * u_ + v_ * v_);

                mCurrentFrame.mvMatch.push_back(keypoints[i]);
