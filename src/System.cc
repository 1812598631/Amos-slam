        			 ...
        			 mbDeactivateLocalizationMode(false)		//没有这个模式转换标志
{
    ...
    mpSeg = new yolact("/home/run/桌面/SLAM/ORB-SLAM-yolact/src/python/yolact_interface.py",
    ...
    mptLoopClosing = new thread(&ORB_SLAM2::LoopClosing::Run,	//线程的主函数
    							mpLoopCloser);					//该函数的参数
    //创建实例分割线程
    //mptSeg= new thread(&ORB_SLAM2::yolact::Run,mpSeg);
    ...
    //设置进程间的指针
    mpSeg->SetTracker(mpTracker);

    mpTracker->SetLocalMapper(mpLocalMapper);
    mpTracker->SetLoopClosing(mpLoopCloser);
    mpTracker->SetSegment(mpSeg);
